...
const ReportTypeGroupChecklists = (): ReactElement => {
  ...
  const [showAssignReportType, toggleShowAssignReportType] = useBooleanState()
  ...
  const removeChild = (reportType: ReportType): void => {
    const result = confirm(`Estás seguro que quieres desaginar el checklist: ${reportType.name ?? ''}`)
    if (!result) return
    ...
    return (
      <section>
        <div className='flex justify-between items-center mb-3 gap-4'>
          ...
          <Button color='primary' onClick={toggleShowAssignReportType}>Asignar Checklist</Button>
        </div>
        {
        ...
        {showAssignReportType && <AssignReportType isOpen={showAssignReportType} onClose={toggleShowAssignReportType}/>}
      </section>
    )
  }
